...
namespace Font {
    ...
    bool fontBufferHeapZeroClearInjector() {
        ...
        if (BytePattern::temp_instance().has_size(1, "Font buffer heap zero clear")) {
            ...
            fontBufferHeapZeroClearHeepAllocJmpAddress =
                Injector::GetBranchDestination(address + 0xC);
    ...
    bool fontBufferClear1Injector() {
        ...
        if (BytePattern::temp_instance().has_size(1, "Font buffer clear")) {
            Injector::WriteMemory<uint8_t>(BytePattern::temp_instance().get_first().address(0x3),
                                           0x10, true);
            return false;
        }
        return true;
    ...
    bool fontSizeLimitInjector() {
        ...
        if (BytePattern::temp_instance().has_size(1, "Font size limit")) {
            Injector::WriteMemory<uint8_t>(BytePattern::temp_instance().get_first().address(0x6),
                                           0x04, true);
            return false;
        }
        return true;
