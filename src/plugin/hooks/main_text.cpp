...
namespace MainText {
	...
	bool mainTextProc1Injector() {
		...
		if (BytePattern::temp_instance().has_size(1, "char fetch fix in text loop 2")) {
	...
	bool mainTextProc2Injector() {
		...
		if (BytePattern::temp_instance().has_size(1, "counter fix in text loop 1")) {
			uintptr_t address = BytePattern::temp_instance().get_first().address();
	...
	bool mainTextProc3Injector() {
		...
		if (BytePattern::temp_instance().has_size(1, "line break return address 2 in text loop 1")) {
		...
		if (BytePattern::temp_instance().has_size(1, "line break fix in text loop 1")) {
			uintptr_t address = BytePattern::temp_instance().get_first().address();
	...
	bool mainTextProc4Injector() {
		...
		if (BytePattern::temp_instance().has_size(1, "char fetch fix in text loop 1")) {
			uintptr_t address = BytePattern::temp_instance().get_first().address();
