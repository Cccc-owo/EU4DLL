...
namespace TooltipAndButton {
    ...
    bool tooltipAndButtonProc5Injector() {
        ...
        BytePattern::temp_instance().find_pattern(
            "44 0F 28 84 24 A0 00 00 00 0F 28 BC 24 B0 00 00 00 48");
        if (BytePattern::temp_instance().has_size(1, "tooltip line break return address 2")) {
            tooltipAndButtonProc5ReturnAddress2 =
                BytePattern::temp_instance().get_first().address();
        } else {
            failed = true;
        }
    ...
    bool tooltipAndButtonProc6Injector() {
        ...
        if (BytePattern::temp_instance().has_size(1, "convert space to non-breaking space")) {
            Injector::WriteMemory<uint8_t>(BytePattern::temp_instance().get_first().address() + 3,
                                           0xA0, true);
            return false;
        }
        return true;
