...
namespace IATHook {
    ...
    inline void* Hook(HMODULE module, const char* target_dll, const char* func_name,
                      void* hook_func) {
        ...
        for (; importDesc->Name != 0; importDesc++) {
            ...
            auto origThunk =
                (PIMAGE_THUNK_DATA)((uintptr_t)module + importDesc->OriginalFirstThunk);
            ...
            for (; origThunk->u1.AddressOfData != 0; origThunk++, iatThunk++) {
                ...
                auto importByName =
                    (PIMAGE_IMPORT_BY_NAME)((uintptr_t)module + origThunk->u1.AddressOfData);
                ...
                DWORD oldProtect;
                VirtualProtect(&iatThunk->u1.Function, sizeof(uintptr_t), PAGE_READWRITE,
                               &oldProtect);
