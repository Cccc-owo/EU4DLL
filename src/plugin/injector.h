...
namespace Injector
{
	...
	struct scoped_unprotect
	{
		...
		{
			if (size > 0) bUnprotected = UnprotectMemory(addr, size, dwOldProtect);
		}

		~scoped_unprotect()
		{
			if (bUnprotected) ProtectMemory(addr, size, dwOldProtect);
		}
	};
	...
	template<class T>
	inline void WriteMemory(uintptr_t addr, T value, bool vp = false)
	{
		scoped_unprotect xprotect((void*)addr, vp ? sizeof(T) : 0);
		*(T*)addr = value;
	}
	...
	inline void MakeRelativeOffset(uintptr_t at, uintptr_t dest, size_t sizeof_addr = 4, bool vp = true)
	{
		switch (sizeof_addr)
		{
	...
	template<typename T>
	inline void MakeJMP(uintptr_t at, T dest, bool vp = true)
	{
	...
	template<typename T>
	inline void MakeCALL(uintptr_t at, T dest, bool vp = true)
	{
		uintptr_t destAddr = (uintptr_t)dest;
		int64_t offset = (int64_t)destAddr - (int64_t)(at + 5);
